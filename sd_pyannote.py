...
class SD_Modified(SpeakerDiarizationMixin, Pipeline):
    ...
    def __init__(
        ...
        super().__init__()
        # to write centroids to file
        self.file_name = file_name
        self.centroids = []
        self.time_offset = -160_000
        self.speaker_map = {}
        IMPORTANT_SPEAKERS = "important_speakers"
        speakers = os.listdir(IMPORTANT_SPEAKERS)
        for i,s in enumerate(speakers):
            self.centroids.append(np.loadtxt(os.path.join(IMPORTANT_SPEAKERS, s)))
            self.speaker_map[str(i)] = s.split(".")[0]
        self.centroids = np.vstack(self.centroids)
        # self.embeddings = np.ndarray()
        self.audio = torch.zeros((1,160_000))
        ...
        self.clustering = clustering_modified.RealTimeAgglomerativeClustering(metric=metric)
    ...
    def initialize(self, num_speakers=None, min_speakers=None, max_speakers=None):
        # self.segments = np.zeros((1,589,3))
        self.num_speakers, self.min_speakers, self.max_speakers = self.set_num_speakers(
    ...
    def add_segment(self,):
        while self.audio.shape[1] >=160_000:
            segment = self.audio[:, 0:160_000]
            file = {"waveform": segment, "sample_rate": 16_000}
            segmentations = self.get_segmentations(file)
            self.segment_window = segmentations.sliding_window
            # self.segments = np.concatenate((self.segments, segmentations.data), axis=0)
            # self.audio = torch.unsqueeze(self.audio[0][:160_000], dim=0)
            ...
            inactive_speakers = np.sum(segmentations, axis=1) == 0
            hard_clusters[inactive_speakers] = -2

            if np.nanmax(count.data) == 0.0:
                # self.discrete_diarization1.append(np.zeros(3))
                self.slide_frame()
                continue
            ...
            # self.discrete_diarization1.append(discrete_diarization.data)
            ...
            text = diarization.to_lab()
            text = text.split("\n")[:-1]
            for t in text:
                time_speaker = t.split(" ")
                time_speaker[0] = str(float(time_speaker[0])*16_000 + self.time_offset)
                time_speaker[1] = str(
                    float(time_speaker[1]) * 16_000 + self.time_offset
                )
                if time_speaker[2] in self.speaker_map:
                    speaker = self.speaker_map[time_speaker[2].strip()]
                else:
                    speaker = "N/A"
                to_send = {
                    "start": time_speaker[0],
                    "end": time_speaker[1],
                    "speaker": speaker,
                }
                print(to_send)
    ...
    def slide_frame(self,):
        shift_by = 16_000
        self.audio = self.audio[:, shift_by:]
        self.time_offset += shift_by

    def add_audio(self,waveform:torch.Tensor):
        self.audio = torch.cat((self.audio, waveform), dim=1)
        self.add_segment()

    def add_embeddings(self, audio:torch.Tensor, segments:torch.Tensor):
        pass

    def write_centroids(self, centroids:np.array,file_name:str):
        print("shape of centroid to write:", centroids.shape)
        np.savetxt(file_name, centroids)
    ...
    def apply(
        ...
        # We only need to keep track of this from 10 seconds ago. Tells us how many speakers at a certain time
        count = self.speaker_count(
        ...
        self.write_centroids(centroids,self.file_name)

        # hard_clusters: (num_chunks, num_speakers)
        # centroids: (num_speakers, dimension)

        # number of detec ted clusters is the number of different speakers
        ...
        if "annotation" in file and file["annotation"]: #IS FALSE
